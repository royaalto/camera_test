...
with open('xml_example/GetUri.xml', 'r') as myfile:
    data = myfile.read()
username = "--"
password = "--"
...
text_file.close()
os.system("curl --silent -X POST --header 'Content-Type: text/xml; charset=utf-8;; action=\"http: // www.onvif.org/ver20/ptz/wsdl/Stop\"' --header 'SOAPAction: \"http: // www.onvif.org/ver20/ptz/wsdl/Stop\"' -d @test.xml 'http://192.168.1.64/onvif/device_service' | xmllint --format -")

...
with open('ContinuousMove.Request.xml', 'r') as myfile:
    data = myfile.read()
username = "admin"
...
print(req_body)
text_file = open("autogen_test.xml", "w")
...
os.system("curl --silent -X POST --header 'Content-Type: text/xml; charset=utf-8;; action=\"http: // www.onvif.org/ver20/ptz/wsdl/Stop\"' --header 'SOAPAction: \"http: // www.onvif.org/ver20/ptz/wsdl/Stop\"' -d @autogen_test.xml 'http://192.168.100.205/onvif/PTZ' | xmllint --format -")
os.system("sleep 1")
with open('Stop.Request.xml', 'r') as myfile:
    data = myfile.read()
username = "admin"
password = "Pilot_vision"
# created = datetime.now().strftime("%Y-%m-%dT%H:%M:%S.000Z")
...
req_body = data.format(username=username, nonce=nonce.decode(
    'utf8'), created=created, digest=digest.decode('utf8'))
print(req_body)
text_file = open("autogen_test1.xml", "w")
text_file.write(req_body)
text_file.close()
os.system("curl --silent -X POST --header 'Content-Type: text/xml; charset=utf-8;; action=\"http: // www.onvif.org/ver20/ptz/wsdl/Stop\"' --header 'SOAPAction: \"http: // www.onvif.org/ver20/ptz/wsdl/Stop\"' -d @autogen_test1.xml 'http://192.168.100.205/onvif/PTZ' | xmllint --format -")
